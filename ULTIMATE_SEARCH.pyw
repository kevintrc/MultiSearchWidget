...
def search_term():
    if(var1.get()==1):
        webbrowser.open("https://www.google.com/search?q="+Term.get()+" torrent",new=2)   
    if(var2.get()==1):
...
Label(root,text="Search Torrent : ",foreground='blue').grid(row=0,column=0,sticky=W,padx=20)
...
var1 = IntVar(root,value=1)
Checkbutton(check, text="Torrent", variable=var1).grid(row=0,column=3,padx=(20,0))
